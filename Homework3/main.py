...
def compute_context_vectors(input_file, language, k, features):
...
def parse_dev_data(input_file, language, k, features, s_data):
...
def test_classifier(filename, language, feats, classifier, clf_data, context_dev, instance_dev):
    # data = parse_data('data/' + language + '-dev.xml')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 5:
        print 'Usage: python main.py <training_file> <test_file> <answer_file> <language>'
    ...
    context_data, target_data, s_data = compute_context_vectors(train, lang, k, ft)
    knn_data, svc_data = train_classifiers(context_data, target_data)
    context_dev, instance_dev = parse_dev_data(test, lang, k, ft, s_data)
    test_classifier(ans, lang, ft, 'answer', svc_data, context_dev, instance_dev)
